...
Claude Code 세션 기록을 자동으로 분석하여, "어제까지 한 일 / 오늘 할 일 / 기한"을 할 일 목록으로 정리해주는 Windows용 TO-DO 관리 프로그램입니다.

프로젝트가 방대해지면 "이게 뭐였지, 어디까지 했지"를 찾는 데 시간이 오래 걸립니다. 이 프로그램의 경우 Claude Code로 작업한 대화 기록이 전부 로컬 파일로 남는다는 점을 이용하여, 그 기록을 매시간 AI로 요약해 할 일 목록으로 관리합니다.
...
- 마지막 분석 이후 새로 쌓인 대화만 증분 분석하며, 새 대화가 없으면 AI를 호출하지 않고 건너뛰기 때문에 사용량 소모는 크지 않습니다. 과거 기록 탐색은 [전체 검색] 버튼을 쓰며, 이 경우 AI를 사용하지 않아 비용이 없습니다.
...
3. 자동 분석(매시간, 새 대화가 있을 때만 AI 실행)을 원하는 경우 명령 프롬프트에서 아래를 한 번 실행합니다.
   ```
...
5. 자동 분석 등록(매시간):
   ```
   schtasks /Create /TN WorkStatusHourly /SC HOURLY /MO 1 /ST 08:50 /F /TR "\"C:\...\pythonw.exe\" \"C:\...\analyze.py\""
   ```
